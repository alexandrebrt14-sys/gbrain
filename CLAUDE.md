...
- `src/core/minions/handlers/shell.ts` — `shell` job handler. Spawns `/bin/sh -c cmd` (absolute path, PATH-override-safe) or `argv[0] argv[1..]` (no shell). Env allowlist: `PATH, HOME, USER, LANG, TZ, NODE_ENV` + caller `env:` overrides + (v0.36.5.0) `inherit:`-resolved keys. UTF-8-safe stdout/stderr tail via `string_decoder.StringDecoder`. Abort (either `ctx.signal` or `ctx.shutdownSignal`) fires SIGTERM → 5s grace → SIGKILL on child. Requires `GBRAIN_ALLOW_SHELL_JOBS=1` on worker (gated by `registerBuiltinHandlers`). **v0.36.5.0:** `ShellJobParams.inherit?: InheritableSecret[]` lets callers request named secrets from the worker's `loadConfig()` to be injected into the child env at spawn time. Names persist in `minion_jobs.data` (and the shell-audit JSONL); values never do. The canonical validator `validateShellJobParams` (sibling file `shell-validate.ts`) runs **pre-enqueue** in both submit surfaces — `gbrain jobs submit shell` (jobs.ts:271) AND `submit_job` op for `name='shell'` (operations.ts:2085). The handler-entry re-validation here is defense-in-depth, catching pre-existing rows and any future submit path that forgets the pre-enqueue call. Closes the codex F-CDX-1 load-bearing bug class where validation in the handler ran AFTER `queue.add()` persisted the row, so the headline "R1 closed" claim was technically false. Also closes the env-secret-in-row leak path PR #1137's workaround opened: `env: { GBRAIN_DATABASE_URL: ... }` and `env: { DATABASE_URL: ... }` are rejected at pre-enqueue with a hint at `inherit: ["database_url"]`.
- `src/core/minions/handlers/shell-inherit.ts` (v0.36.5.0, NEW) — single-source-of-truth `INHERITABLE` record. One object entry per inheritable secret; the type `InheritableSecret`, `INHERITABLE_NAMES`, `ALL_SHADOW_KEYS`, `inheritedByShadowKey()` reverse-lookup, and `isInheritableSecret()` type guard all derive from it. Scope v0.36.5.0: `database_url` only (envKey `GBRAIN_DATABASE_URL`, shadowKeys `['GBRAIN_DATABASE_URL', 'DATABASE_URL']`). API keys (anthropic/openai/groq/voyage) deferred to a follow-up PR pending `GBrainConfig` + `buildGatewayConfig()` refactor since `groq_api_key` and `voyage_api_key` aren't first-class config fields today. Adding a new key in the follow-up = one object entry; the type system and validation walk pick it up automatically. No parallel-array drift bug class.
- `src/core/minions/handlers/shell-validate.ts` (v0.36.5.0, NEW) — `validateShellJobParams(data, opts?)` shared pre-enqueue validator. Throws `UnrecoverableError` with paste-ready operator hints on every failure path. Five rules in order: (1) existing cmd/argv/cwd/env shape, (2) inherit array shape + closed enum, (3) env-shadow rejection (any env key in `ALL_SHADOW_KEYS` rejected regardless of inherit), (4) explicit shadow message when `inherit:[X]` + `env:{shadowKey(X)}` collide, (5) F1 fail-fast on missing config value with `gbrain config set <key>` hint. Test seam: `opts.config` lets unit tests drive the validator hermetically without mocking the module. The defense-in-depth re-call at `shell.ts` handler entry catches pre-existing rows submitted before v0.36.5.0.
